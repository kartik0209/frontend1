...
const allMacros = [
  ...
  { name: '{publisher_id}', description: 'ID of Publisher who is running the Campaign' },
  ...
  { name: '{aff_name}', description: 'Name of Publisher running the Campaign' },
  { name: '{aff_username}', description: 'Username of Publisher running the Campaign' },
  ...
  { name: '{aff_chain}', description: 'Use this macro to pass subids in the publisher postback' },
  ...
  { name: '{conv_rp_currency}', description: 'Currency in which the revenue is reflected' },
  ...
  { name: '{tdomain}', description: 'Tracking domain on which the Campaign Runs' },
  ...
  { name: '{creative_name}', description: 'Get the creative name in tracking link' },
...
const handleMacroClick = (macroName) => {
  ...
  console.log('Current URL:', formState.defaultCampaignUrl);
  
  updateFormState(prevState => {
    const newUrl = (prevState.defaultCampaignUrl || "") + macroName;
    console.log('New URL will be:', newUrl);
    return { defaultCampaignUrl: newUrl };
  });
};
...
<TextArea
  ...
  onChange={(e) => {
    ...
    updateFormState({ 
      ...formState,
      defaultCampaignUrl: e.target.value 
    });
  }}
/>
...
<div className="url-tokens-section" style={{ marginBottom: '16px' }}>
  <div style={{ marginBottom: '8px', fontWeight: '500' }}>Most Used URL tokens</div>
  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
    {['{click_id}', '{camp_id}', '{publisher_id}', '{source}', '{gaid}'].map((token) => (
      <Tag 
        key={token} 
  ...
  <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '8px' }}>
      {allMacros.map((macro) => (
        ...
        >
          <div style={{ fontWeight: '500', color: '#1890ff' }}>{macro.name}</div>
