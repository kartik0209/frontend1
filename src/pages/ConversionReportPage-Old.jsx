...
const ConversionReportsPageOld = ({ name }) => {
  ...
  const handlePublisherChange = (publisherIds) => {
    ...
    const filterQuery = buildPublisherAdvertiserQuery(
      publisherIds,
      selectedAdvertisers
    );
 ...
 const buildPublisherAdvertiserQuery = (publisherIds, advertiserIds) => {
  ...
  const fetchAllReports = async (
    ...
    const allFilters = [filterQuery, additionalFilters]
      .filter(Boolean)
      .join("&");
  ...
  const handleTableChange = (paginationInfo) => {
    ...
    if (selectedCampaign) {
      ...
      fetchAllReports(current, pageSize, filterQuery);
    }
  };
    ...
    const baseColumns = [
      {
        ...
        sorter: (a, b) =>
          new Date(a.clickTime || 0) - new Date(b.clickTime || 0),
        width: 120,
        style: { fontSize: "12px" },
      },
      {
    ...
    if (selectedPublishers && selectedPublishers.length > 0) {
      baseColumns.push({
        ...
        sorter: (a, b) => {
          const aVal = (
            a.publisher?.name ||
            a.campaignTracking?.publisher?.name ||
            ""
          )
            .toString()
            .toLowerCase();
          const bVal = (
            b.publisher?.name ||
            b.campaignTracking?.publisher?.name ||
            ""
          )
            .toString()
            .toLowerCase();
    ...
    if (selectedAdvertisers && selectedAdvertisers.length > 0) {
      baseColumns.push({
        ...
        sorter: (a, b) => {
          const aVal = (
            a.advertiser?.name ||
            a.campaignTracking?.advertiser?.name ||
            ""
          )
            .toString()
            .toLowerCase();
          const bVal = (
            b.advertiser?.name ||
            b.campaignTracking?.advertiser?.name ||
            ""
          )
            .toString()
            .toLowerCase();
    ...
    baseColumns.push(
      ...
      },
      {
      ...
      },
       {
        ...
        render: (value, record) => {
          if (!value || value === 0) return "N/A";
          const currency = record.revenue || "$";
          return `${parseFloat(value).toFixed(2)}`;
        },
        sorter: (a, b) =>
          (parseFloat(a.revenue) || 0) -
          (parseFloat(b.revenue) || 0),
        align: "right",
        width: 130,
        style: { fontSize: "12px" },
      },
       {
        ...
        render: (value, record) => {
          if (!value || value === 0) return "N/A";
          const currency = record.payout || "$";
          return `${parseFloat(value).toFixed(2)}`;
        },
        sorter: (a, b) =>
          (parseFloat(a.payout) || 0) -
